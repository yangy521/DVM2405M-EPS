...
/* MOTOR SPEED LEVEL */
#define _MOTOR_SPEED_LEVEL100			1
#define _MOTOR_SPEED_LEVEL200			2
#define _MOTOR_SPEED_LEVEL300			3
#define _MOTOR_SPEED_LEVEL700			4
#define _MOTOR_SPEED_LEVEL1500		5
#define _MOTOR_SPEED_LEVEL3000		6
#define _MOTOR_SPEED_LEVEL4000		7
...
typedef struct SPEED_PID
{
	...
	_iq		VfKoe;		/* Koe between voltage and spd*/
	_iq		VBusKoe;		/* Koe between voltage and spd*/
	INT64S	Ui;				/* Variable: 64 bit Integral output */
	INT32S	UpSucc;		/* Variable: 64 bit Proportional remain */
	_iq  	TorqueOld;		/* Variable: Pre-saturated output */
...
#define   SPD_CURLIMIT_ERVS		 	(0x0001 << 5)
#define   SPD_CURLIMIT_ALL      (SPD_CURLIMIT_DIRVE | SPD_CURLIMIT_UPHILL | SPD_CURLIMIT_DWHILL | SPD_CURLIMIT_REGEN | SPD_CURLIMIT_BRAKE | SPD_CURLIMIT_ERVS)
...
#define   SPD_LOOP_ERVS					(0x0001 << 21)
#define   SPD_LOOP_RESET_SMT		(0x0001 << 22)
#define   SPD_LOOP_THROTTLE_RVS	(0x0001 << 23)
...
//*********** PLC spd point
	...
	#define MIN_SPD_CMD   			32									//最小速度命令 rpm
	#define RDY_STOP_FRQ   			20									//停止转动转速 rpm
	#define RDY_STAT_FRQ   			30									//恢复电机控制的转动转速 rpm
	#define RAMP_FRQ            20									//溜坡转速 rpm
	#define SOFT_LIMIT_FRQ			60.0							//速度接近目标值，需要柔和加减速处理的区域
#endif
#if (MOTOR_SPEED_LEVEL	== _MOTOR_SPEED_LEVEL700)
	#define MIN_SPD_CMD   			64									//最小速度命令 rpm
	#define RDY_STOP_FRQ   			40									//停止转动转速 rpm
	#define RDY_STAT_FRQ   			60									//恢复电机控制的转动转速 rpm
	#define RAMP_FRQ            40									//溜坡转速 rpm
	#define SOFT_LIMIT_FRQ			100.0							//速度接近目标值，需要柔和加减速处理的区域
#endif
#if (MOTOR_SPEED_LEVEL	== _MOTOR_SPEED_LEVEL1500)
	#define MIN_SPD_CMD   			100									//最小速度命令 rpm
	#define RDY_STOP_FRQ   			45									//停止转动转速 rpm
	#define RDY_STAT_FRQ   			60									//恢复电机控制的转动转速 rpm
	#define RAMP_FRQ            45									//溜坡转速 rpm
	#define SOFT_LIMIT_FRQ			150.0							//速度接近目标值，需要柔和加减速处理的区域
#endif
#if (MOTOR_SPEED_LEVEL	== _MOTOR_SPEED_LEVEL3000)
#ifndef MIN_SPD_CMD
	#define MIN_SPD_CMD   			150									//最小速度命令 rpm
#endif
//	#define RDY_STOP_FRQ   			90									//停止转动转速 rpm
//	#define RDY_STAT_FRQ   			120									//恢复电机控制的转动转速 rpm
//	#define RAMP_FRQ            90									//溜坡转速 rpm
//	#define SOFT_LIMIT_FRQ			300.0							//速度接近目标值，需要柔和加减速处理的区域
//	#define MIN_SPD_CMD   			240									//最小速度命令 rpm
	#define RDY_STOP_FRQ   			120									//停止转动转速 rpm
	#define RDY_STAT_FRQ   			180									//恢复电机控制的转动转速 rpm
	#define RAMP_FRQ            120									//溜坡转速 rpm
	#define SOFT_LIMIT_FRQ			300.0							//速度接近目标值，需要柔和加减速处理的区域
#endif
#if (MOTOR_SPEED_LEVEL	== _MOTOR_SPEED_LEVEL4000)
//	#define MIN_SPD_CMD   			150									//最小速度命令 rpm
//	#define RDY_STOP_FRQ   			90									//停止转动转速 rpm
//	#define RDY_STAT_FRQ   			120									//恢复电机控制的转动转速 rpm
//	#define RAMP_FRQ            90									//溜坡转速 rpm
//	#define SOFT_LIMIT_FRQ			300.0							//速度接近目标值，需要柔和加减速处理的区域
	#define MIN_SPD_CMD   			320									//最小速度命令 rpm
	#define RDY_STOP_FRQ   			160									//停止转动转速 rpm
	#define RDY_STAT_FRQ   			240									//恢复电机控制的转动转速 rpm
	#define RAMP_FRQ            160									//溜坡转速 rpm
	#define SOFT_LIMIT_FRQ			400.0							//速度接近目标值，需要柔和加减速处理的区域
#endif
#define REF_FOLLOW_LIMIT(X)		    (X>>4)		//停止制动过程中，最小制动力
...
typedef struct SPEED_LOOP
{
	...
	INT64S		IqFdbSum;
	_iq				IqFdbFilter;
#endif //(IQ_ARITHMETIC_TYPE	== _IQ_ARITHMETIC_OPENLOOP_CURLIMIT)
	_iq  			KpDefault;				/* Parameter: Proportional gain */
	_iq  			KiDefault;				/* Parameter: Integral gain */
	_iq  			KcDefault;				/* Parameter: Integral correction gain */
	_iq  			KpSoft;				/* Parameter: Proportional gain */
	_iq  			KiSoft;				/* Parameter: Integral gain */
	_iq  			KcSoft;				/* Parameter: Integral correction gain */
	...
	_iq				VoltageBus2s;			//dc bus voltage 2s even
	...
	INT32U BatteryLostCountL;
	INT32U BatteryLostCountH;
