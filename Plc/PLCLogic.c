...
//	}
	...
	for (index=0; index < (sizeof(gPLCCtl.diDataIn.ucIn)/sizeof(gPLCCtl.diDataIn.ucIn[0])); index++)
	{
		...
		if(gPLCCtl.AutoManuelMode==0) //Manuel Mode
		{
			...
			else if((gPLCCtl.diDataIn.ucIn[SWI1_R]==0) && (SL_CHK(PLC_TER_EN_PWM)) ) //无互锁 且 自检完成
			{
				if(gPLCCtl.SafeLockHome == 0)
					gCRam.bEnaHard = 0;
				else
				{
					if(netTimer[Timer_SoftEnaDelay].bIsStart == false)
					{
						SetNetTimer(Timer_SoftEnaDelay, 100); //delay 100ms
					}
					else
					{
						if(netTimer[Timer_SoftEnaDelay].bIsOvertime == true)
						{
							gCRam.bEnaHard = 1;
							//KillNetTimer(Timer_SoftEnaDelay);
						}					
					}					
...
//				gCRam.NewMod = CTRL_NULL;	
	...
	#endif
	if((gPLCCtl.diDataIn.ucInNew[SWI2_R] == gPLCCtl.diDataIn.ucInNew[SWI3_R]) && (gPLCCtl.ReHomeSwi == 1))  //如果sw2和sw3状态一致 则回中信号有误
	{
...
//	gPara.AcMotorSpeed = (gCRam.SpeedAvgAddSum*60000/gPLCCtl.enc)>>8;//rpm	
	...
	gPara.AcMotorSpeed = gCRam.SpeedAvgAdd*(60000/8)/gPLCCtl.enc;
	...
	gPara.TimetoSpd1 = (gCurrentSample.V12_5OvCurrent * OUT5V_STD_CURRENT)/4096;
	gPara.TimetoSpd2 = (gCurrentSample.V12_5OvCurrent2 * OUT5V_STD_CURRENT)/4096;
