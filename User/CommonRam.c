...
//	gPara.AcMotorDriveCurrentLimitRatio;
  ...
  gPLCCtl.MaxPWMOut = MAX_PWM;
    ...
    gPLCCtl.ReMotorEnc = (gPara.RedundantParaConfig >> 0) & 0x01;        /* Bit0: 1:双电机编码器       0:单电机编码器 */
    gPLCCtl.ReCmdEnc = (gPara.RedundantParaConfig >> 1) & 0x01;          /* Bit1: 1:双编码器方向盘     0:单编码器方向盘 */
    gPLCCtl.RePotentiometer= (gPara.RedundantParaConfig >> 2) & 0x01;   /* Bit2: 1:双电位计           0:单电位计*/
    gPLCCtl.ReFault = (gPara.RedundantParaConfig >> 3) & 0x01;           /* Bit3: 1:有Faultout过流     0:无Faultout过流*/
    gPLCCtl.ReHomeSwi = (gPara.RedundantParaConfig >> 4) & 0x01;          /* Bit4: 1:冗余回中开关判断    0:不冗余判断*/
}
