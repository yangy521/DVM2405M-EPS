...
//	float fTemp;
    ...
    static INT16U ThrottleOffdelay = 0;
	...
	if(gPLCCtl.ThrottleType==0)//油门类型1：0V~2.5V~5V
	{
		...
		else
		{
			gCRam.CmdPercent = 0;
			gCRam.CmdDirection= 0;
            if(ThrottleOffdelay < 600)  //delay Precison: 1ms
            {
                ThrottleOffdelay++;
            }
            else
...
{	
	...
	/*** FaultOut输出过流检测 ***/
	if((gPLCCtl.diDataIn.ucIn[FAULTOUT_R] != 0) && (gPLCCtl.ReFault == 1)) //如果检测到过流信号  会重启电流检测芯片reset脚
	{	
		if(FaultErrDelay > 70)
		{
			gPLCCtl.doDataOut[FAULT_OUT] = 1;  //断开fault
            gPLCCtl.doDataOut[FAULT_OUT_RESET] = 0;  //不复位
